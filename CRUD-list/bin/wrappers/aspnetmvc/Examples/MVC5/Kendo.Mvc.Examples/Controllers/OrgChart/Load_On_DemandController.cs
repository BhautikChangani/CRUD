...
namespace Kendo.Mvc.Examples.Controllers
{
    ...
    {
        [Demo]
        public ActionResult Load_On_Demand()
        {
            return View();
        }
        public JsonResult LoadOnDemandData([DataSourceRequest] DataSourceRequest request, int? id)
        {
            var source = GetData();
            var loaded = source.Where(e => e.ParentID == id)
                .Select(e => new OrgChartEmployeeViewModel
                {
                    ID = e.ID,
                    Title = e.Title,
                    Name = e.Name,
                    Avatar = e.Avatar,
                    ParentID = e.ParentID,
                    hasChildren = source.Where(s => s.ParentID == e.ID).Count() > 0
                });
