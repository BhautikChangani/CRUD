...
namespace Kendo.Mvc.Examples.Controllers
{
    public partial class OrgChartController : Controller
    {
        ...
        public JsonResult EventsRead([DataSourceRequest] DataSourceRequest request)
        {
            return Json(new
            {
                Data = EventsOrgChartData
            }, JsonRequestBehavior.AllowGet);
        }
        public JsonResult EventsCreate(OrgChartEmployeeViewModel model)
        {
            int lastID = EventsOrgChartData.Select(m => m.ID).Max();
            if (model.ID == 0)
            {
                model.ID = lastID + 1;
            }
        ...
        public JsonResult EventsDestroy(OrgChartEmployeeViewModel model)
        {
            var target = EventsOne(m => m.ID == model.ID);
            EventsOrgChartData.Remove(target);

            return Json(target);
        }
        public JsonResult EventsUpdate(OrgChartEmployeeViewModel model)
        {
            var target = EventsOne(m => m.ID == model.ID);
        ...
        public static OrgChartEmployeeViewModel EventsOne(Func<OrgChartEmployeeViewModel, bool> predicate)
        {
            return EventsOrgChartData.FirstOrDefault(predicate);
        }
        private static IList<OrgChartEmployeeViewModel> EventsOrgChartData
        {
            get
            {
                IList<OrgChartEmployeeViewModel> source = System.Web.HttpContext.Current.Session["EventsOrgChartEmployees"] as IList<OrgChartEmployeeViewModel>;

                if (source == null)
                {
                    System.Web.HttpContext.Current.Session["EventsOrgChartEmployees"] = source = new List<OrgChartEmployeeViewModel>
                    {
