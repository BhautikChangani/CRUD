namespace Kendo.Mvc.Examples.Models.TreeList
{
    ...
    public class EmployeeDirectoryService
    {
        ...
        public EmployeeDirectoryService(DemoDBContext context)
        {
            db = context;
        }
        ...
        {
            var data = db.EmployeeDirectory.ToList().OrderBy(x => x.ReportsTo);

            return data.Select(employee => employee.ToEmployeeDirectoryRemoteModel(db.EmployeeDirectory.Where(s => s.ReportsTo == employee.EmployeeID).Count() > 0)).ToList();
        }
