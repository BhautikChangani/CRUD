namespace Kendo.Mvc.Examples.Models.Scheduler
{
    ...
    public class SchedulerMeetingService : ISchedulerEventService<MeetingViewModel>
    {
        ...
        public SchedulerMeetingService(DemoDBContext context)
        {
            db = context;
        }
        ...
        public virtual IList<MeetingViewModel> GetAll()
        {
            ...
            if (result == null || UpdateDatabase)
            {
                result = db.Meetings.ToList().Select(meeting => new MeetingViewModel
                {
                    ...
                    Attendees = meeting.MeetingAttendees.Select(m => (int)m.AttendeeID).ToArray()
                }).ToList();
        ...
        public virtual IList<MeetingViewModel> GetHierarchicalMeetings()
        {
            ...
            if (result == null || UpdateDatabase)
            {
                ...
                .Where(m => (m.RoomID == 1 && m.MeetingAttendees.All(a => firstRoomAttendees.Contains((int)a.AttendeeID))) ||
                            (m.RoomID == 2 && m.MeetingAttendees.All(a => secondRoomAttendees.Contains((int)a.AttendeeID))))
